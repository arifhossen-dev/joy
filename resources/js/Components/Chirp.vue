...
<template>
  <div class="flex p-6 space-x-2">
    <svg xmlns="http://www.w3.org/2000/svg" class="w-6 h-6 text-gray-600 -scale-x-100" fill="none" viewBox="0 0 24 24"
    ...
    <div class="flex-1">
      <div class="flex items-center justify-between">
        ...
        <Dropdown v-if="chirp.user.id === $page.props.auth.user.id">
          ...
          <template #content>
            <button
              class="block w-full px-4 py-2 text-sm leading-5 text-left text-white transition duration-150 ease-in-out hover:bg-gray-500 focus:bg-gray-100"
              @click="editing = true">
              Edit
            </button>
      ...
      <form v-if="editing"
        ...
        <textarea v-model="form.message"
          class="w-full mt-4 text-gray-900 border-gray-300 rounded-md shadow-sm focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50"></textarea>
